...
#define SERVO Servo8Bit // can be "Servo" for another board
...
static const int full = 180;  // esc wants to see throttle at zero while it boots
static const int init_delay = 2000;
static const int zero = 0; // full-speed is 180
static const int slowdown = 2; // min 1
...
void setup() {
    esc.attach(SERVO_PIN, 1000, 2000);  // attaches the servo object
    esc.write(full);                    // full throttle
    diag.init(LED_PIN);
}
...
void loop() {
    ...
    if (initialized) {
        ...
        if (now < init_delay) {
            ...
            if (full - increment > zero) {
                esc.write(full - increment);
